...
function cityWeather() {
  ...
  $.ajax({
    ...
    $.ajax(
      `https://api.openweathermap.org/data/2.5/uvi?lat=${response.coord.lat}&lon=${response.coord.lon}&appid=${apiKey}`
    ).then((uvResp) => {
    ...
    let mainData = `<h2>${curName} (${today})<img src="https://openweathermap.org/img/w/${curIcon}.png"></img></h2>
